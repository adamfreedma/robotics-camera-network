...
class GUI(threading.Thread):
    ...
    def clear_cone(self, x: int, y: int):
        """
        ...
        if self.panel and self.paint_dc:
            ...
            self.paint_dc.SetPen(wx.Pen('gray'))
            self.paint_dc.SetBrush(wx.Brush('gray'))
            self.paint_dc.DrawCircle(x, y, self.OBJECT_RADIUS)
            self.paint_lock.release()

    def draw_cone(self, x: int, y: int):
        """
        draws a cone (yellow circle) on the screen
        :param x:
        :param y:
        ...
        if self.panel and self.paint_dc and [x, y] != self.prev_cone_pos:
            self.clear_cone(*self.prev_cone_pos)
            print("draw", x, y)
            self.paint_lock.acquire()
